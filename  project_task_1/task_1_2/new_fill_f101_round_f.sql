-- PROCEDURE: dm.fill_f101_round_f(date)
...
begin
	...
	cte2 as(
	 select from_date,
            ...
            turn_cre_total,
		   (case
			   when characteristic LIKE 'A' AND currency_code in ('643', '810')
			   then balance_in_rub - turn_cre_rub + turn_deb_rub
			   when characteristic LIKE 'P' AND currency_code in ('643', '810')
			   then balance_in_rub + turn_cre_rub - turn_deb_rub
			   else 0
		    end)                  as balance_out_rub,

           (case
			   when characteristic LIKE 'A' AND currency_code not in ('643', '810')
			   then balance_in_val - turn_cre_val + turn_deb_val
			   when characteristic LIKE 'P' AND currency_code not in ('643', '810')
			   then balance_in_val + turn_cre_val - turn_deb_val
			   else 0
	...
	SELECT *, balance_out_rub + balance_out_val as balance_out_total
    FROM cte2;
...
$BODY$;
ALTER PROCEDURE dm.fill_f101_round_f(date)
    OWNER TO postgres;